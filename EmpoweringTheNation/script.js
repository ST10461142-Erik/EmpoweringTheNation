...
// Initialize EmailJS
(function () {
    emailjs.init("Je-kAzGw14iIjfraM");
})();

// Function to calculate total and send email
function calculateTotalAndSendEmail() {
    const checkboxes = document.querySelectorAll(".course-checkbox");
    let total = 0;
    let selectedCourses = [];

    // Gather selected courses and calculate total
    checkboxes.forEach((checkbox) => {
        if (checkbox.checked) {
            const courseName = checkbox.parentElement.textContent.trim();
            const coursePrice = parseFloat(checkbox.getAttribute("data-price"));
            total += coursePrice;
            selectedCourses.push(courseName);
        }
    });

    // Error handling: no courses selected
    if (selectedCourses.length === 0) {
    ...
    totalFeeDisplay.innerHTML = `
        ...
        <p>Discounted Total: R${discountedTotal.toFixed(2)}</p>
        <p>VAT (15%): R${vatAmount.toFixed(2)}</p>
        <p><strong>Final Total: R${finalTotal.toFixed(2)}</strong></p>
    `;

    // Save calculation data to localStorage
    localStorage.setItem("calculatedData", JSON.stringify({
        total: total.toFixed(2),
        discountedTotal: discountedTotal.toFixed(2),
        vatAmount: vatAmount.toFixed(2),
        finalTotal: finalTotal.toFixed(2),
    ...
    emailjs.send("service_dono4fl", "template_bo5j5oh", templateParams)
...
// Event listener for "Calculate and Send Email" button
document.getElementById("calculate-btn").addEventListener("click", calculateTotalAndSendEmail);

// Run sync function on page load
window.onload = syncCheckBoxesWithCart;
